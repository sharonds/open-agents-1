...
- If a provider-specific bash tool emits a different tool part key (for example `tool-bash_anthropic`), keep TUI approval inference/matching logic aligned so command-prefix "don't ask again" rules still apply.
- Normalize direct model provider identifiers before toolset selection (for example `anthropic.messages` -> `anthropic`) so provider-specific toolsets are selected correctly.
