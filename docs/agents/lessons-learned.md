...
- Durable workflow chat runs must reuse the existing assistant message id when a request resumes an in-progress assistant turn (for example after `ask_user_question` input). If the workflow always emits a fresh start id like `workflowRunId`, the UI forks the assistant turn into a duplicate message instead of appending to the pending one.
